  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        `${BASE_URL}/api/v1/subscription/create-subscription`,
        {
          userName: userName.trim(),
          planType,
          planAmount,
          planId,
          planName // Include plan name in the request
        },
        {
          headers: {
      ...
      if (res.data?.success) {
        ...
        const errorMessage = res.data?.message || "Failed to process subscription";
    ...
    <section className='bg-gray-900'>
      <div className='container mx-auto px-6'>
        <form className='flex w-full h-screen justify-center items-center flex-col gap-5' onSubmit={onSubmit}>
          ...
          <div className="w-full max-w-[750px]">
            <label className="block text-white text-sm font-bold mb-2" htmlFor="planname">
              Selected Plan
            </label>
            <Input
              id="planname"
          ...
          <button type='submit' className='btn px-5 py-2 font-normal outline-none border border-white rounded-sm text-xl text-white hover:text-black hover:bg-white transition-all ease-in w-full max-w-[750px]'>Pay Now</button>
        </form>
      </div>
